...
void addrelFunction(string ent1Name, string ent2Name, string relName)
{
    ...
    if(relsArray == NULL) //non ho relazioni
    {
        ...
        if(ent2->maxIn == 0)       //se ent2 non ha ancora archi entranti alloco
        {
            ...
            indxIn = binEntSearchInsert(ent1Name, ent2, relsArray[indxRel]->relName, true);         //restituisce l'indice del successivo (se -1 comando non valido)
...
int main()
{
    ...
    f2 = freopen("output.txt", "w", stdout);
    while(1) {
        //printf("%d\n", riga);
        processInput();
        //riga++;
